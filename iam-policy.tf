resource "aws_iam_policy" "aws_loadbalancer_controller" {
  ...
  path        = "/"
  description = "Allows access to resources needed to run kubernetes AWS LoadBalancer Controller."
...
resource "aws_iam_role_policy_attachment" "aws_loadbalancer_controller" {
  ...
  role       = var.ec2_role_name == null ? aws_iam_role.aws_loadbalancer_controller[0].name : var.ec2_role_name
}
